...
@section('table')
    @foreach ($replays as $replay)
        <tr class="data-rows {{ method_exists($replay, 'trashed') && $replay->trashed() ? 'deleted-table-row' : '' }}" data-replay-id="{{ $replay->id }}">
            @if (!(method_exists($replay, 'trashed') && $replay->trashed()))<td class="dt-checkboxes-cell"><input type="checkbox" value="{{ $replay->id }}" class="dt-checkboxes form-check-input"></td>@else<td></td>@endif
