...
@section('table')
    @foreach ($districts as $district)
        <tr class="data-rows {{ method_exists($district, 'trashed') && $district->trashed() ? 'deleted-table-row' : '' }}" data-district-id="{{ $district->id }}">
            @if (!(method_exists($district, 'trashed') && $district->trashed()))<td class="dt-checkboxes-cell"><input type="checkbox" value="{{ $district->id }}" class="dt-checkboxes form-check-input"></td>@else<td></td>@endif
            <td>{{ $district->name }}</td>
            <td>{{ $district->city?->name ?? '—' }}</td>
            <td>@if(!(method_exists($district, 'trashed') && $district->trashed()))<div class="form-check form-switch mb-0 d-flex justify-content-center"><input class="form-check-input switch-active" type="checkbox" role="switch" data-id="{{ $district->id }}" data-route="{{ route('admin.districts.switchIsActive', ['id' => $district->id]) }}" {{ $district->is_active ? 'checked' : '' }}></div>@else<span class="text-muted">—</span>@endif</td>
            <td>
