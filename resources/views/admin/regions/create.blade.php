...
@push('content')
<form id="createRegionForm" class="mb-3 validated-form form" novalidate method="POST" action="{{ route('admin.regions.store') }}" enctype="multipart/form-data">
    @csrf
    <div class="admins-form-section">
        <div class="row g-3">
            <x-form.select :options="['name' => 'country_id', 'label' => 'country', 'class' => 'col-md-12', 'isRequired' => true, 'options' => App\Models\Country::get()->map(fn($c) => ['id' => $c->id, 'name' => $c->name])]" />
