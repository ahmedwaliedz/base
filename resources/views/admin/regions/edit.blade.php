...
<form id="editRegionForm" class="mb-3 validated-form form" novalidate method="POST" action="{{ route('admin.regions.update', ['region' => $region]) }}" enctype="multipart/form-data">
    ...
    <div class="admins-form-section">
        <div class="row g-3">
            <x-form.select :options="['name' => 'country_id', 'label' => 'country', 'class' => 'col-md-12', 'isRequired' => true, 'value' => $region->country_id, 'options' => $countries]" />
    ...
    <div class="admins-form-section">
        <div class="row g-3">
            <x-form.text :options="['name' => 'name', 'label' => 'name', 'class' => 'col-md-12', 'isRequired' => true, 'isMultiLanguage' => true, 'value' => $region->getTranslationsArray()]" />
        </div>
    </div>
