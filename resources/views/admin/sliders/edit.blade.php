...
<form id="editSliderForm" class="mb-3 validated-form form" novalidate method="POST" action="{{ route('admin.sliders.update', ['slider' => $slider]) }}" enctype="multipart/form-data">
    ...
    <div class="admins-form-section">
        <div class="row g-3">
            <x-form.text :options="['name' => 'title', 'label' => 'title', 'class' => 'col-md-12', 'isMultiLanguage' => true, 'value' => $slider->title]" />
            <x-form.text-area :options="['name' => 'description', 'label' => 'description', 'class' => 'col-md-12', 'isMultiLanguage' => true, 'value' => $slider->description]" />
        </div>
    </div>
