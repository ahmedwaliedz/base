<div class="crud-toolbar d-flex justify-content-between flex-wrap align-items-center gap-3">
    <div class="d-flex align-items-center flex-wrap gap-2">
        ...
        @if($hasNotification)
            <button type="button" data-bs-toggle="modal" data-bs-target="#notificationModal" data-id="group" class="send-notification btn btn-outline-secondary waves-effect">
                <span class="ti-xs ti ti-bell-plus me-1"></span>{{ __('admin/main.send_notification') }}
            </button>
        @endif

        @if($hasEmail)
            <button type="button" data-bs-toggle="modal" data-bs-target="#emailModal" class="btn btn-outline-secondary waves-effect">
                <span class="ti-xs ti ti-mail-plus me-1"></span>{{ __('admin/main.send_email') }}
            </button>
        @endif

        @if($hasReload)
            <button type="button" class="btn btn-outline-secondary waves-effect reload" aria-label="{{ __('admin/main.reload') }}">
                <span class="ti-xs ti ti-reload me-1"></span>{{ __('admin/main.reload') }}
            </button>
        @endif

        @if($hasDeleteAll)
            <button data-route="{{$deleteAllRoute}}" type="button" class="btn btn-outline-danger waves-effect delete-all-button">
    ...
    <div class="d-flex align-items-center gap-2">
        @if($hasFilter)
            <button type="button" class="btn btn-outline-secondary waves-effect show_filter" aria-label="{{ __('admin/main.search') }}">
