<div class="d-flex justify-content-between">
    <div class="">
        @if($createRoute)
            <a href="{{ $createRoute }}" type="button" class="me-1 mt-2 btn btn-outline-primary waves-effect">
                <span class="ti-xs ti ti-table-plus me-1"></span>{{ __('admin/main.create') }}
            </a>
        @endif

        @if($hasNotification)
            <button type="button" data-bs-toggle="modal" data-bs-target="#notificationModal" class="me-1 mt-2 btn btn-outline-warning waves-effect">
                <span class="ti-xs ti ti-bell-plus me-1"></span>{{ __('admin/main.send_notification') }}
            </button>
        @endif

        @if($hasEmail)
            <button type="button" data-bs-toggle="modal" data-bs-target="#notificationModal" class="me-1 mt-2 btn btn-outline-info waves-effect">
                <span class="ti-xs ti ti-mail-plus me-1"></span>{{ __('admin/main.send_email') }}
            </button>
        @endif

        @if($hasDelete)
            <button type="button" class="me-1 mt-2 btn btn-outline-danger waves-effect delete-all-button d-none">
                <span class="ti-xs ti ti-trash-off me-1 "></span>{{ __('admin/main.delete_selected') }}
            </button>
        @endif

        @if($hasReload)
            <button type="button" class="me-1 mt-2 btn btn-outline-success waves-effect reload">
        ...
        @if($hasExport && ($exportPdf || $exportExcel || $exportWord || $exportJson || $exportCopy))
            <div class="btn-group me-1">
                <button type="button" class="btn mt-2 btn-outline-primary dropdown-toggle waves-effect" data-bs-toggle="dropdown" aria-expanded="false">
    ...
    @if($hasFilter)
        <div>
            <button type="button" class="btn mt-2 btn-outline-info waves-effect show_filter">
