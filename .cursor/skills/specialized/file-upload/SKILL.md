---
name: file-upload
description: Add file, image, or document uploads using Spatie Media Library. Trigger for upload features.
---
...
Handle upload in Service class (`app/Services/Admin/` or `app/Services/{Domain}/`):
...
| Component | Location |
|-----------|----------|
| Media Library Config | `config/media-library.php` if published; package defaults otherwise |
...
Do not require publishing `config/media-library.php` unless the task needs customization. Use package defaults when no published config exists.
...
// (do nothing â€” Spatie preserves existing media)
```
