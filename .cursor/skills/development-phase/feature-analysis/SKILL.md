...
description: Analyze a new feature and produce an implementation plan. Trigger at the start of any feature, module, API, UI, or backend change.
---
...
- Check `app/Services/Admin/` for existing admin services
- Check `app/Services/{Domain}/` (e.g., `app/Services/Countries/`) for API services
...
- New Service in `app/Services/Admin/` or `app/Services/{Domain}/`?
- Controller changes in `app/Http/Controllers/Admin/` or `app/Http/Controllers/Api/V1/`?

### Step 5: Identify Frontend Changes

- New Blade views in `resources/views/admin/`?
- New routes in `routes/admin.php` or `routes/api/v1/<topic>.php`?
...
| API | `app/Services/{Domain}/` (e.g., `app/Services/Countries/`) | - |
...
### Type of Feature
- **API-only?** â†’ API endpoints, resources, validation
- **UI-only?** â†’ Blade views, controller, routes
- **Both?** â†’ Full stack (API + UI)

### New or Extension
- **New module?** â†’ Create Service, Controller, Model, etc.
- **Extension?** â†’ Extend existing service, add routes
...
- Follow architecture: Controller â†’ Form Request â†’ Service â†’ Model