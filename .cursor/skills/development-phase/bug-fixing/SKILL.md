---
name: bug-fixing
description: Find root cause and apply the smallest safe fix for a bug. Trigger for urgent bugs, regressions, or failing tests.
---
...
### Service Layer
- Check `app/Services/Admin/` or `app/Services/{Domain}/`
- Business logic errors
- Missing error handling

### Response Handling
- Check `app/Traits/Response/` for response structure
...
### Authentication
- Check middleware in routes (admin routes use `CheckRolePermission`)
- Verify route-name-to-permission mapping for admin routes
- Check record-level policies only if the project uses `app/Policies/`
...
| Auth failures | Middleware, route-name/permission mapping, record-level policies (if used) |
...
| âŒ Wrong (Symptoms) | âœ… Right (Root Cause) |
|-------------------|---------------------|
